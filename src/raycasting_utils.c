...
static double	get_side(double ray_dir, double map, double delta_dist,
					double pos);
...
void	init_ray_info(t_data *cub, t_ray *ray, int x)
{
	...
	ray->side_x = get_side(ray->dir_x, ray->map_x, ray->delta_x,
			cub->player->pos_x);
	ray->side_y = get_side(ray->dir_y, ray->map_y, ray->delta_y,
			cub->player->pos_y);
}
